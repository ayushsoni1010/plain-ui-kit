...
export default function Header({ headerConfiguration }) {
  const [logoData] = useState({
    ...
    className: headerConfiguration.logo.className
      ? headerConfiguration.logo.className
      : "",
    shape: headerConfiguration.logo.logoShape
      ? headerConfiguration.logo.logoShape
      : "",
  });
  const [styleConfiguration] = useState({
    backgroundColor: headerConfiguration.themeConfig.backgroundColor,
    shadow: headerConfiguration.themeConfig.shadow
      ? "header-shadow"
      : "NO_SHADOW",
          ...
          {headerOptionsRef.map((option, index) => {
            if (option.type === "link") {
              if (option.route === undefined) {
                return (
                  <p className="header-option__text" key={index}>
                    {option.name}
                  </p>
                );
              } else {
